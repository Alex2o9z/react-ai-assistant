...
const MultiSidebarContext = React.createContext(null);
...
function MultiSidebarProvider({
    ...
    style,
    children,
    leftMobileBehaviorOnTablet = false,
    rightMobileBehaviorOnTablet = false,
    ...props
}) {
    ...
    const leftOpenState = leftOpen ?? _leftOpen;
    const setLeftOpen = React.useCallback(
        (value) => {
            const openState = typeof value === "function" ? value(leftOpenState) : value;
            if (onLeftOpenChange) {
        ...
        [onLeftOpenChange, leftOpenState]
    );

    // Right Sidebar State
    const [_rightOpen, _setRightOpen] = React.useState(defaultRightOpen);
    const rightOpenState = rightOpen ?? _rightOpen;
    const setRightOpen = React.useCallback(
        (value) => {
            const openState = typeof value === "function" ? value(rightOpenState) : value;
        ...
        [onRightOpenChange, rightOpenState]
    );
    ...
    const toggleLeftSidebar = React.useCallback(() => {
        // Toggle openMobile if mobile or tablet with mobileBehaviorOnTablet
        return isMobile || (isTablet && leftMobileBehaviorOnTablet)
    ...
    const toggleRightSidebar = React.useCallback(() => {
        // Toggle openMobile if mobile or tablet with mobileBehaviorOnTablet
        return isMobile || (isTablet && rightMobileBehaviorOnTablet)
            ? setRightOpenMobile((open) => !open)
            : setRightOpen((open) => !open);
    }, [isMobile, setRightOpen, setRightOpenMobile]);

    // Sidebar contexts
    const leftSidebarContext = React.useMemo(
        () => ({
            state: leftOpenState ? "expanded" : "collapsed",
            open: leftOpenState,
            setOpen: setLeftOpen,
            ...
            side: "left",
            mobileBehaviorOnTablet: leftMobileBehaviorOnTablet, // Pass to context
        }),
        [leftOpenState, setLeftOpen, leftOpenMobile, isMobile, toggleLeftSidebar]
    );

    const rightSidebarContext = React.useMemo(
        () => ({
            state: rightOpenState ? "expanded" : "collapsed",
            open: rightOpenState,
            ...
            side: "right",
            mobileBehaviorOnTablet: rightMobileBehaviorOnTablet, // Pass to context
        }),
        [rightOpenState, setRightOpen, rightOpenMobile, isMobile, toggleRightSidebar]
    );
...
function Sidebar({
    ...
    const { [side === "left" ? "leftSidebar" : "rightSidebar"]: sidebarContext } =
        useMultiSidebar();
    const { isMobile, state, openMobile, setOpenMobile, mobileBehaviorOnTablet } = sidebarContext;
    const isTablet = useIsTablet();
    const useSheet = isMobile || (mobileBehaviorOnTablet && isTablet);
    ...
    return (
        <div
            ...
            <div
                ...
                className={cn(
                    "fixed inset-y-0 z-10 hidden h-svh w-(--sidebar-width) group-data-[side=right]:w-(--right-sidebar-width) transition-[left,right,width] duration-200 ease-linear md:flex",
                    side === "left"
                ...
                <div
                    ...
                    className={cn(
                        "bg-sidebar flex h-full w-full flex-col group-data-[variant=floating]:border-sidebar-border group-data-[variant=floating]:rounded-lg group-data-[variant=floating]:border group-data-[variant=floating]:shadow",
...
const sidebarTriggerVariants = cva(
    "data-sidebar='trigger' data-slot='sidebar-trigger' inline-flex items-center justify-center",
    {
        variants: {
            variant: {
...
function SidebarTrigger({
    ...
    side = "left",
    variant = "default",
    showOn = "all", // "all", "tablet-and-below", "mobile-only"
    children,
    ...props
}) {
    const { [side === "left" ? "leftSidebar" : "rightSidebar"]: sidebarContext } =
    ...
    // Determine visibility based on showOn prop
    const isVisible = (() => {
        switch (showOn) {
            case "all":
                return true;
            case "tablet-and-below":
    ...
    return (
        <Button
            variant="ghost"
            className={cn(sidebarTriggerVariants({ variant, side }), className)}
            onClick={(event) => {
                onClick?.(event);
                toggleSidebar();
            }}
...
// const MultiSidebarProvider = forwardRef(({
//     defaultLeftOpen = true,
//     defaultRightOpen = true,
